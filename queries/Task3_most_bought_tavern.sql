WITH minion_rank AS (
	SELECT 
	bmo.player_tavern_tier , 
	bmo.dbf_id, 
	COUNT(bmo.dbf_id) AS minion_count,
	RANK() OVER(PARTITION BY bmo.player_tavern_tier ORDER BY minion_count DESC) AS minion_ranking
	...
	GROUP by bmo.player_tavern_tier,  bmo.dbf_id
	ORDER BY bmo.player_tavern_tier , minion_count DESC
) 
select
minion_rank.player_tavern_tier,
c2.name AS minion, 
...
ORDER BY minion_rank.player_tavern_tier , minion_rank.minion_count desc
;